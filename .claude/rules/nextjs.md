...
- Media (video, artifacts, thumbnails) goes through `lib/media` — **no route may read the filesystem for media**. Addresses are derived from identity (`lib/media/keys.ts`), never from a folder name, and the driver behind them is either the local filesystem or Supabase Storage. `SWINGSAGE_MEDIA_ROOT` now names only the *analyzer's working directory*, read by the publish step and nothing else. Don't assume `next/image` remote-pattern config or an image CDN exists unless you've checked `next.config.ts`.
