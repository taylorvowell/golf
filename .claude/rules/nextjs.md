...
- API routes under `src/app/api/**` follow doc 02's job/polling contract (see root `CLAUDE.md`) — job state lives in Postgres (`jobs` table), not in-memory across requests, except the in-process map that mirrors an actively-running job so the hot per-frame stdout path never round-trips the DB.
