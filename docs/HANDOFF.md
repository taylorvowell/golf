...
| OPEN | Read the overlay's frame-lock off the S25+ (2 numbers, 1 minute) | Physical device interaction — the phone is not connected and `adb shell input` is blocked while it is covered | The geometry half of Gate 3 is already done here and passes on all ten fixtures (`RUNBOOK` §12a). What is left is the *frame lock with the trace on*, which only the device can answer. **Turn on Wireless debugging, uncover the phone, then:** open SwingSage → any swing → shake → **Reload** (JS only, no rebuild) → read **Overlay drift** in the Frame sync panel with the **Club head trace** chip ON and again with it OFF. Those two figures answer the Skia question. Full steps: `RUNBOOK` §12b. |
