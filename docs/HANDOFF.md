...
| OPEN | **Tap `Run 250 seeks` on a swing and read two lines back** — 30 seconds, `RUNBOOK.md` §11 | Physical device interaction | The player is confirmed working on the S25+ (2026-08-12): a swing plays, 59.9 fps UI, 0 stutters, container fps 60.00 vs 60 declared, and a partial sweep read **30 seeks · 100.0% exact · p95 0 · max 0** before the phone was picked up. `mobile-player` step 01 needs the full n≥200 run to close. Open a swing, scroll to **Frame sync**, tap **Run 250 seeks**, wait ~30s, report *Seeks exact (JS)* and *Seeks exact (native)*. |
