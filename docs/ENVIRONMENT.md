...
| Address | **STATIC: `10.0.1.25`** (Taylor reserved it on the router, 2026-08-26 — "it will always be this"). MAC `0c:32:3a:68:b3:83`. **The wireless-debug PORT still randomizes on every toggle** — Android's design, not fixable without root — so discovery is a port scan of this one host (`adb-phone.mjs` rung 3). The adb PAIRING (`taylo@TAYLOR` / `taylo@TAYLOR-DESKTOP` on the phone's paired-devices list) persists across toggles, which is why `adb connect` never needs re-pairing. Before 2026-08-26 the phone was on DHCP (`.123`, earlier `.125`) — old logs and cache files may still name those. |
...
usually still live after a PC reboot); a TCP scan of **30000–50000 on the static `10.0.1.25`**
(~20s worst-case, deterministic while wireless debugging is on); then `adb mdns services` and the
whole-ARP-table sweep as the fallback for the day the reservation moves.
...
| **A WEDGED dev-client process swallows every relaunch, and reads as "stale cached bundle"** — the process stays alive with its React context never ready (`ReactHost: Tried to access onNewIntent while context is not ready` repeating in `logcat --pid`), so each `am start` deep-link lands in the same dead process and no JS runtime ever registers with Metro (`/json/list` stays `[]`) while reverse mapping, Metro and the pid all look healthy. Cost three launch attempts on 2026-08-26. | `adb -s <serial> shell am force-stop com.swingsage.spike`, then `pnpm --filter mobile phone`. Diagnose it from the pid: the SAME pid surviving across launch attempts is the tell — a healthy relaunch mints a new one. |
...
| **Non-interactive stops need `--yes`** | `modal container stop` / `modal app stop` prompt for confirmation and piping `y` does not satisfy them ("no interactive terminal detected"). Also: stopping a CONTAINER only makes Modal's class-level `retries` respawn the task — to actually kill runaway jobs, `modal app stop --yes <app-id>` then redeploy (same app name keeps the QStash-facing URL). Done 2026-08-26 to stop three runaway imports |
