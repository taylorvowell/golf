...
| OPEN | **Uncover the phone and confirm the player on screen** — 30 seconds, procedure in `RUNBOOK.md` §11 | Physical device interaction | The S25+'s proximity sensor is covered, so Samsung's *Accidental touch protection* (`UnintentionalLcdOn`) swallows **every** `adb shell input` event and no automation can dismiss it — `settings put system screen_off_pocket 0`, `dumpsys sensorservice restrict` and force-stopping `com.samsung.android.gesture` were all tried and all failed. Blocks the last item of `mobile-player` step 01: open a swing, tap **Run 250 seeks**, and read the two exactness lines off the Frame sync panel. |
