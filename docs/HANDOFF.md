...
| OPEN | Reinstall the dev build on the S25+ (adds `react-native-svg`) | Physical device interaction | The after-swing gauges added the app's first SVG native module, so the dev client on your phone is now stale — it will crash on the after-swing screen until reinstalled. When the phone is next adb-connected (wireless debugging on): `cd apps/mobile && npx expo run:android --device` — or just ask Claude with the phone connected. The emulator already runs the new build. |
