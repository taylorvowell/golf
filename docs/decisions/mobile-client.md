...
### Every activity is locked to portrait, not just `MainActivity`

**Decision:** `plugins/withPortraitOnly.js` stamps `android:screenOrientation="portrait"` onto
every activity that arrives from a library manifest — the dev-launcher, its auth webview and error
screen, RN's dev-settings screen, `expo-video`'s fullscreen player, and the image-picker cropper.
The app never rotates, on any screen, in any build.
**Gotchas:** `expo.orientation: "portrait"` in `app.json` only reaches the app's own
`MainActivity`. Library activities declare nothing, inherit `unspecified`, and follow the sensor.
The dev-client launcher is the one that shows: it is the first screen on every debug launch, so
the app appears to unlock the phone's rotation lock before its own UI is ever on screen. The stubs
merge by `android:name` with `tools:node="merge"`, so they add the attribute rather than replacing
the library's declaration. Landing it is a prebuild + native rebuild — it is manifest config, not
JS.
...
### The orbit is the logo swoosh's own ellipse, fitted — never its bounding box

**Decision:** `SWING_PLANE` in `src/design/system/orbitGeometry.ts` holds the ellipse the logo's
swoosh actually draws, in mark user units, and every loader takes its centre, tilt and squash from
there. It is generated by `apps/mobile/scripts/fit-swing-plane.mjs`, which rasterises the swoosh,
walks the ridge of its distance field to recover a centreline, fits a conic to that, and writes a
PNG of the fit over the artwork. Balls travel it with `SPIN = -1` — up the far side, down the near
one. Loader marks drop the AI slabs; the lettering is lockup furniture and does not survive being
shrunk to spinner size.
**Gotchas:** The swoosh is two FILLED shapes, so there is no stroke to read a centreline off — the
loaders previously fitted an ellipse to its BOUNDING BOX and then set lean and squash by eye at
-22° and 0.62. The artwork's own are **-35.65° and 0.432**, so the orbit shared only a centre with
the thing it was meant to trace, and no amount of tuning those two knobs closes a gap of that
shape. Look at the PNG before believing a re-fit: the fit reports a median error, and a median is
exactly the statistic a wrong-but-plausible ellipse scores well on. `SPIN` multiplies the ANGLE, so
the near/far handover and the size ramp — both read off `sin` of that same angle — reverse with it;
negating a position instead reverses travel and leaves the depth cues pointing the old way.
...
### Home's hero deck is the spotlight carousel; dismissal is server-persisted and generic

**Decision:** The Home sheet's first slot is a center-snapped, both-neighbours-peeking,
infinitely looping carousel of dismissable promo/feature cards ("spotlights"), in three
deliberately separate layers. (1) **`SnapCarousel`** (`design/system/`) is the house carousel —
purely presentational, core `ScrollView` only (`snapToInterval` + `disableIntervalMomentum`;
gesture-handler stays excluded per D47), a 3-copy deck with an at-rest unanimated rebase for the
loop, centering via contentContainer padding (never iOS-only `contentInset`), and the dismiss X
rendered by the FRAME so every card earns the same affordance. Deck sizes are behaviors: 0
renders null, 1 is static, 2+ loops. (2) **Dismissal** is the generic `user_dismissals` store
(`/api/v1/dismissals` + `useDismissals`): namespaced versioned keys (`spotlight.multiview.v1`),
an AsyncStorage mirror so a dismissed card never flashes at cold start, optimistic dismiss with
an offline replay queue, and re-showing a reworked card is a key version bump — never an
un-dismissal. (3) **The registry** (`features/spotlights/registry.tsx`) is code-defined curation
order; eligibility is a pure function of a context built only from data Home already holds
(entitlement tier/can, swing/session counts, account age) — eligibility must never cost a
request — plus an always-empty `triggers` set reserved for server-granted event cards later.
The old stacked `DeepIntroCard`/`StanceIntroCard` live in the deck now; their device-local
dismissals replay into the server store once and the legacy hooks are gone.
**Gotchas:** card CTAs pass `pressDelayMs={SCROLL_PRESS_DELAY_MS}` (Button grew the prop for
this); the Pro card's visibility is the same `tier` read as Profile's upgrade door — capability
gates still go through `can()`.
**See:** `.claude/feature-tracks/spotlights/DESIGN-spotlights.md`;
`docs/decisions/platform-data.md` (the store).

## Brand rendering

### The lockup is generated, and the swing gradient has one definition

`src/design/system/brandPaths.ts` is generated from `apps/mobile/assets/brand/swingsage-logo.svg`
by `scripts/make-brand.mjs`. Never hand-edit it; replace the SVG and run the generator, then run
`make-icons.mjs`, which reads the same source but not the same file.

The swoosh's gradient is **not in the SVG** — the source paints the arc a flat colour. The ramp is
`SWING_STOPS` / `SWING_STOPS_ON_LIGHT` in the generated file, and `SwingGradient` in `BrandLogo.tsx`
is the only component that draws it. Anything else needing that gradient imports `SwingGradient`;
a second hand-written copy is how the loader drifted a shade away from the mark it had to match.

Its `id` is per instance (`useId`), not module-scoped: react-native-svg keeps one global registry
of gradient ids, so a shared constant lets the last component to mount redefine the gradient every
earlier one is still referencing.

### Light and dark carry different ramps and different inks, on purpose

| | Wordmark | Golfer | AI slabs | Arc |
|---|---|---|---|---|
| Light | `#172B4E` | the arc's ramp | `#2DF0FB` | `#0D94DB → #2DF0FB` |
| Dark | `#FFFFFF` | `#FFFFFF` | `#2DF0FB` | `#1E5F9E → #0D94DB → #2DF0FB` |

Chosen from a thirty-six-way review. The light ramp stays inside the bright half because it paints
the GOLFER as well as the arc, and a filled shape shows the whole gradient at once where a stroke
only crosses a slice — so a deep first stop that reads as depth on the arc reads as a shadow half on
the figure, splitting it into two objects. The wordmark's ink sits beside it already saying "dark".

A caller passing `color="#FFFFFF"` is over a photo or a hero, so the lockup takes the dark pairing
whatever the theme says.

The splash icon is that same light lockup with the type cropped off — figure and arc on
`SWING_STOPS_ON_LIGHT` — so the loading mark and the header are one drawing.

`BRAND_INK` (`#1C0032`) survives only as the artwork's SENTINEL fill, never as a paint. It is a
purple that arrived with the source SVG and was never chosen.

### The golf ball's dimple count scales with its rendered size

`GolfBall` draws a Fibonacci lattice, and the count is derived from `size` rather than fixed. A
fixed count is only correct at the size it was tuned at: at 14 px a full-density ball is 400-odd
sub-pixel SVG nodes that composite to flat grey — invisible individually and ruinous in aggregate.
The style guide shows around 68 balls at once and asked for ~29,000 circles, which stopped it
opening. Below `EMBOSS_FROM` a dimple is one circle instead of a light/dark pair, because the two
halves need to be a pixel apart to read as an indentation at all.

### The `coach` brand icon IS the logomark

`BrandIcon name="coach"` resolves from `brandPaths`, not from `brandIconPaths`. The supplied copy
was traced from an older revision and went stale silently the first time the logo changed. The tab
bar, the coach screen and `CoachLoader` all follow with no call-site change.

### One loading spinner: `SwingLoader`

The golfer with a golf ball orbiting on the 30° swing plane, in `design/system/SwingLoader.tsx`.
Four properties are load-bearing rather than decorative, and each was found by rejecting the
alternative:

- **Occlusion by paint order** — the ring is cut at its vertices and drawn far half, ball, near
  half, figure. Squash alone reads as a flat oval.
- **The figure above the near arc** — the club is a thin diagonal that the front stroke otherwise
  cuts through, and the border round the figure is the GROUND's colour so it reads as a gap.
- **The ball's depth swap is biased off the halfway point.** The geometric boundary sits exactly at
  the near arc's end-caps, so the ball spent its approach to each vertex behind the front stroke.
- **One turn is 850 ms.** It is flashed while a section loads, so a full cycle has to land inside
  the flash; a two-second orbit only ever shows a fragment.
- **The plane is MEASURED, not chosen.** `SWING_PLANE` in `orbitGeometry.ts` is fitted to the
  swoosh's own centreline by `scripts/fit-swing-plane.mjs`, so the ball walks the artwork rather
  than an ellipse that merely shares its bounding box — the box-fitted version was out by 14° of
  lean and half the squash, and no amount of tuning by eye closes a gap of that shape.

`orbitGeometry.ts` holds the plane and the geometry for walking it, and nothing else. The loader's
own look — ramp, stroke weights, track colours — lives in `SwingLoader.tsx`. Having had both in
both files is how two copies of the same constant ended up a paste apart.

It takes `SWING_STOPS_ON_LIGHT` on BOTH grounds — the light lockmark's ramp, not the default one and
not a per-ground pick. That ramp suits a spinner for the reason it suits the light logomark: it has
to paint a FILLED figure as well as a stroke, and a filled shape shows the whole gradient at once, so
a deep first stop reads as depth on an arc but as a shadow half on the golfer. Only the border and
the track change with the ground, and neither carries brand meaning.

Everything it drives is a transform or an opacity, so the orbit stays on the native driver; the
position is a sampled interpolation because animating the SVG circle's `cx`/`cy` would force it
onto JS.
