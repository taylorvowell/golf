...
export interface SwingSummary {
  ...
  /**
   * Whether the golfer starred this swing (§7.3). Additive (D41) — absent on a response from a build older than the field, which a client reads as false. Set through PATCH /api/v1/swings/:id; it is a property of the SWING and not of a device, so a star survives a reinstall and reaches every phone the golfer signs in on.
   */
  favourite?: boolean;
}
/**
...
/**
 * PATCH /api/v1/swings/:id. Partial, like the session and profile patches — a screen sends only what it edits, so a build written before a field existed cannot erase it. Owner only: a coach may read a swing and may never restyle the golfer's own log.
 *
 * This interface was referenced by `Api`'s JSON-Schema
 * via the `definition` "swingPatchRequest".
 */
export interface SwingPatchRequest {
  favourite?: boolean;
}
/**
 * The swing as it now stands. Answered in full rather than with 204 so the client writes its cache from the CONFIRMED row instead of from what it hoped it sent.
 *
 * This interface was referenced by `Api`'s JSON-Schema
 * via the `definition` "swingPatchResponse".
 */
export interface SwingPatchResponse {
  swing: SwingSummary;
}
/**
