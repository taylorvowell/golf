...
| OPEN | Read the overlay's frame-lock off the S25+, and eye the rebuilt swing screen | Physical device interaction — the phone is not connected and `adb shell input` is blocked while it is covered · plus a design judgement call | Two things in one visit. **(a) The measurement:** the geometry half of Gate 3 is done here and passes on all ten fixtures (`RUNBOOK` §12a); what is left is the *frame lock with the trace on*, which only the device can answer — read **Overlay drift** at the bottom of the **Metrics** panel with the **Club head trace** tile ON and again with it OFF (`RUNBOOK` §12b). **(b) The look:** the swing screen is full-bleed with a thumbnail scrubber, a floating dock and drag-resizable slide-up panels (Overlays, Compare, Metrics, Analysis); the scrubber, the overlay tiles and the Compare panel are the parts worth a verdict. **A rebuild is now required first** — the 2026-08-12 perf-audit execution changed the frame-clock native module and the app's native config (`.claude/audits/mobile-rn-perf-2026-08-12/_status.md`), so a JS reload shows a stale native half. **Turn on Wireless debugging, uncover the phone, run `adb connect IP:PORT`, then tell Claude "phone's connected"** — the rebuild+install (`npx expo run:android`) and the number-reading prep are Claude's to run; the two numbers and the design verdict are yours. |
